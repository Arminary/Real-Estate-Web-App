    ...
    <main className='p-3 max-w-4xl mx-auto'>
      ...
      <form onSubmit={handleSubmit} className='flex flex-col sm:flex-row gap-4'>
        <div className='flex flex-col gap-4 flex-1'>
            <input type="text" placeholder='Title' className='border rounded-lg p-3' id='title' maxLength='62' minLength='10' required onChange={handleChange} value={formData.title} />
            ...
            <div className='flex flex-wrap gap-6'>
                ...
                <div className='flex gap-2'>
                    <input type="checkbox" id='parkingSpot' className='w-5' onChange={handleChange} checked={formData.parking}/>
        ...
        <div className='flex flex-col flex-1 gap-4'>
          ...
          <div className='flex gap-4'>
            ...
            onChange={(e)=> setFiles(e.target.files)}
              ...
              accept='image/*'
              multiple />
              <button type="button" disabled={uploading} onClick={handleImageSubmit} className='p-3 text-green-700 border border-green-700 hover:shadow-lg disabled:opacity-80'>{uploading ? 'Uploading...' : 'Upload'}</button>
            </div>
              <p className='text-red-600 text-sm'>{imageUploadError && imageUploadError}</p>
              {
                formData.imageUrls.length > 0 && formData.imageUrls.map((url, index) =>(
                  <div key={url} className="flex justify-betweenp-3 border items-center">
                    ...
                    <button type="button" onClick={ () => handleRemoveImage(index)} className="pl-56 text-red-700 rounded-lg uppercase hover:opacity-75">Delete</button>
                  </div>
                ))
              }
